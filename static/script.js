...
const NodeTemplates = {
  [NodeTypes.DATA]: `
        Data Input
        <input type="file" accept=".csv" onchange="event.preventDefault();">
      `,
  [NodeTypes.EXPLORE]: `
        Data Exploration
        <select>
    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        ...
        try {
          ...
          userData = rows; // Store the raw data
          
          // Send the raw CSV data for analysis
          const response = await fetch('http://localhost:5000/analyze_dataset', {
            method: 'POST',
            headers: {
          ...
          if (data.success) {
            ...
            // Create pipeline from the suggestion
            const pipeline = JSON.parse(data.pipeline);
            let xPos = 100;
            
            // Create nodes for each step in the pipeline
            pipeline.forEach((step) => {
              const node = createNode(step.type, xPos, 200);
              if (node.querySelector('select')) {
                node.querySelector('select').value = step.operation;
              }
              // Add explanation as a tooltip
              if (step.explanation) {
                node.title = step.explanation;
              }
              xPos += 200;
            });
            
            // Show dataset analysis in the code panel
            document.getElementById('generatedCode').textContent = 
              'Dataset Analysis:\n' + 
              JSON.stringify(data.data_info, null, 2) + 
              '\n\nSuggested Pipeline:\n' + 
              data.pipeline;
...
// Add this function after the parseCSV function
async function suggestPipeline(datasetDescription) {
  try {
    const response = await fetch('http://localhost:5000/suggest_pipeline', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ description: datasetDescription })
    });
    ...
    if (data.success) {
      ...
      // Create nodes based on suggested pipeline
      let xPos = 100;
    ...
    console.error('Error suggesting pipeline:', error);
    alert('Failed to suggest pipeline: ' + error.message);
  }
}
...
async function handleChatInput() {
    ...
    if (userData) {
        try {
            ...
            const data = await response.json();
            if (data.success) {
                addMessage(data.pipeline);
            }
        } catch (error) {
            addMessage('Sorry, I encountered an error analyzing your request.');
        }
    } else {
        addMessage('Please upload a dataset first!');
    }
}
...
// Modify the file upload handler to include chat feedback
document.addEventListener("change", (e) => {
    if (e.target.type === "file") {
        // ...existing file upload code...
        
        // Add this inside the success block after processing the dataset
        addMessage("Dataset uploaded and analyzed! Ask me questions about the pipeline or dataset.");
    }
});
